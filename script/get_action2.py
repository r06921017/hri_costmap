...
if __name__ == '__main__':
    ...
    for _fin in os.listdir('/home/rdaneel/hri_costmap_traj/A107/'):
        if _fin.split('.')[-1] == 'csv':
            file_name = '/home/rdaneel/hri_costmap_traj/A107/' + _fin
            ...
            for idx, row in traj_df.iterrows():
                traj_df.loc[idx, 'f1'] = np.around(getDist(row['x'+r_id], row['y'+r_id], row['z'+r_id], row['x'+h_id], row['y'+h_id], row['z'+h_id]), around_digit)
                traj_df.loc[idx, 'f2'] = np.around(getDist(row['x'+r_id], row['y'+r_id], row['z'+r_id], goal_pos2[0], goal_pos2[1], goal_pos2[2]), around_digit)
